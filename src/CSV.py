...
class CSV:
    ...
    def add_empty_col(self, property: str, missing_value: str="XXX"):
        values = [missing_value for _ in self.entries]
        self.add_col(property, values)
        return self
    ...
    def show(self, n_entries: int=5, min_colsize: int=3, max_colsize: int=20, max_linesize: int=200, round_digit: int=4) -> None:
        """Show summary of CSV."""
        ...
        print(self)
        for line in lines:
            print_line(line, sizes=col_sizes, max_linesize=max_linesize, round_digit=round_digit)
